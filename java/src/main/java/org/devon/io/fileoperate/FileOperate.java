 ...
 * 
 * @author 杨彩 http://blog.sina.com.cn/m/yangcai 文件操作 1.0
 */
...
public class FileOperate {
	...
	public void newFolder(String folderPath) {
		...
		try {
			...
			if (!myFilePath.exists()) {
				boolean isMake = myFilePath.mkdir();
			}
	...
	public void delFile(String filePathAndName) {
		try {
			...
			boolean isDelete =myDelFile.delete();
	...
	public void copyFolder(String oldPath, String newPath) {
		...
		try {
			File nefile =new File(newPath);
			boolean isMakeFile = nefile.mkdirs(); // 如果文件夹不存在 则建立新文件夹
