...
import com.ecas.module.systemmanager.dto.LoginDTO;
import com.ecas.module.systemmanager.entiy.SessionInfo;
...
public class LoginManagerController extends BaseController<IUserService,User> {
    ...
    public String doLogout(LoginDTO login, HttpSession session, HttpServletRequest request) {
    ...
    public String login(LoginDTO login, ModelMap model, String verifyCode, HttpSession session, HttpServletRequest request) {
        ...
        try {
            if (null != login.getUserName() && null != login.getPassword()) {//判断是否录入用户名和密码
                ...
                if (null != login) {//获取用户信息成功
                    login.setLoginTime(System.nanoTime());//更新当前登录时间
                    login.setLoginIp(request.getRemoteHost());//获取登录IP地址
                    login.setStatus(1);//获取登录状态
